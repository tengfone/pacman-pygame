...
################## HELPER FUNCTIONS ##################
    ...
    def load(self):
        self.background = pygame.image.load('./black.jpg')
            ...
            for yidx, line in enumerate(file):
                for xidx, char in enumerate(line):
                    if char == '1':
                        ...
                        pygame.draw.rect(self.background, BLACK, (xidx*self.cell_width,
                                         yidx*self.cell_height, self.cell_width, self.cell_height))
    ...
    def draw_grid(self):
        ...
        for wall in self.walls:
            pygame.draw.rect(self.background, WALL_COLOR, (wall.x * self.cell_width,
                             wall.y*self.cell_height, self.cell_width, self.cell_height))

        for coin in self.coins:
            pygame.draw.rect(self.background, COIN_COLOR, (coin.x * self.cell_width,
    ...
    def draw_coins(self):
        for coin in self.coins:
            pygame.draw.circle(self.screen, COIN_COLOR, (int(coin.x*self.cell_width) + self.cell_width // 2 +
                               TOP_BOTTOM_BUFFER // 2, int(coin.y*self.cell_height) + self.cell_height // 2 + TOP_BOTTOM_BUFFER // 2), 5)
    ...
    def get_highscore(self):
        try:
            loadHighScore = open("highscore.pickle", "rb")
            loadHighScore = pickle.load(loadHighScore)
...
################## START FUNCTIONS ##################
    ...
    def start_draw(self):
        ...
        self.draw_text('PUSH SPACE BAR TO PLAY', self.screen, [WIDTH//2, HEIGHT//2 - 50],
                       START_TEXT_SIZE, (170, 132, 58), START_FONT, centered=True)
        self.draw_text('PUSH A TO ENTER AI MODE', self.screen, [WIDTH//2, HEIGHT//2 - 0],
                       START_TEXT_SIZE, (170, 132, 58), START_FONT, centered=True)
        self.draw_text('1 PLAYER ONLY', self.screen, [WIDTH//2, HEIGHT//2 + 50],
                       START_TEXT_SIZE, (44, 167, 198), START_FONT, centered=True)
        self.draw_text('HIGHSCORE: {}'.format(self.highscore), self.screen, [4, 0],
...
################## PLAYING FUNCTIONS ##################
    ...
    def playing_draw(self):
        ...
        self.draw_text('CURRENT SCORE: {}'.format(self.player.current_score), self.screen, [
                       60, 0], 18, WHITE, START_FONT, centered=False)
        self.draw_text('HIGH SCORE: {}'.format(self.highscore), self.screen, [
                       WIDTH//2 + 60, 0], 18, WHITE, START_FONT, centered=False)
        self.player.draw()
        for enemy in self.enemies:
            enemy.draw()
        pygame.display.update()

################## RL (Reinforce Learning) FUNCTIONS ##################
...
################## GAMEOVER FUNCTIONS ##################
    ...
    def gameover_draw(self):
        ...
        if (self.player.current_score > self.highscore):
            ...
            pickle.dump(self.player.current_score, pickle_out)
            pickle_out.close()
